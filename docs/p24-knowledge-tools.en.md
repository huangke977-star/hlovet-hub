...
The main model and each capability can have an optional fallback model with separate regular-input/cached-input/output prices. Fallback occurs only for timeouts, temporary connectivity failures, HTTP 429, or HTTP 5xx; primary failures and fallback results are recorded separately and estimated with the price of the model actually used, while parameter and authorization errors are not retried. Cache-hit units are recorded only when the provider reports them; otherwise they remain unknown.
