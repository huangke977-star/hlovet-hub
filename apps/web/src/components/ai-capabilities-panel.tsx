      ...
      {items.map((item) => {
        ...
        return <article className="ai-capability-card" key={item.capability}>
          ...
          <div className="ai-pricing-grid">
            <label><span>{phrase("输入价 / 百万 tokens", "Input price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update(item.capability, "inputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={draft.inputCostPerMillionMicros / 1000000} /></label>
            <label><span>{phrase("备用输入价 / 百万 tokens", "Fallback input price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update(item.capability, "fallbackInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={(draft.fallbackInputCostPerMillionMicros ?? 0) / 1000000} /></label>
            <label><span>{phrase("缓存输入价 / 百万 tokens", "Cached input price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update(item.capability, "cachedInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={(draft.cachedInputCostPerMillionMicros ?? 0) / 1000000} /></label>
            <label><span>{phrase("备用缓存输入价 / 百万 tokens", "Fallback cached input price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update(item.capability, "fallbackCachedInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={(draft.fallbackCachedInputCostPerMillionMicros ?? 0) / 1000000} /></label>
            <label><span>{phrase("输出价 / 百万 tokens", "Output price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update(item.capability, "outputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={draft.outputCostPerMillionMicros / 1000000} /></label>
            <label><span>{phrase("备用输出价 / 百万 tokens", "Fallback output price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update(item.capability, "fallbackOutputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={(draft.fallbackOutputCostPerMillionMicros ?? 0) / 1000000} /></label>
          </div>
          <div className="ai-capability-numbers">
            <label><span>{phrase("全站并发", "Global")}</span><AiNumberInput min={1} max={8} onChange={(event) => update(item.capability, "globalConcurrency", Number(event.target.value))} value={draft.globalConcurrency} /></label>
            <label><span>{phrase("单用户", "Per user")}</span><AiNumberInput min={1} max={8} onChange={(event) => update(item.capability, "userConcurrency", Number(event.target.value))} value={draft.userConcurrency} /></label>
            <label><span>{phrase("每日上限", "Daily limit")}</span><AiNumberInput min={0} onChange={(event) => update(item.capability, "dailyRequestLimit", Number(event.target.value))} value={draft.dailyRequestLimit} /></label>
            <label><span>{phrase("单次价格", "Unit price")}</span><AiNumberInput min={0} onChange={(event) => update(item.capability, "unitCostMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={draft.unitCostMicros / 1000000} /></label>
          </div>
