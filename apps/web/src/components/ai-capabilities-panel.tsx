    ...
    <div className="ai-capability-grid">
      {items.map((item) => {
        ...
        return <article className="ai-capability-card" key={item.capability}>
          ...
          <div className="ai-capability-fields ai-capability-pricing"><label><span>{phrase("输入价 / 百万 tokens", "Input price / million tokens")}</span><input min={0} onChange={(event) => update(item.capability, "inputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={draft.inputCostPerMillionMicros / 1000000} /></label><label><span>{phrase("缓存输入价 / 百万 tokens", "Cached input price / million tokens")}</span><input min={0} onChange={(event) => update(item.capability, "cachedInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.cachedInputCostPerMillionMicros ?? 0) / 1000000} /></label><label><span>{phrase("输出价 / 百万 tokens", "Output price / million tokens")}</span><input min={0} onChange={(event) => update(item.capability, "outputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={draft.outputCostPerMillionMicros / 1000000} /></label></div>
          <div className="ai-capability-fields ai-capability-fallback-pricing"><label><span>{phrase("备用输入价 / 百万 tokens", "Fallback input price / million tokens")}</span><input min={0} onChange={(event) => update(item.capability, "fallbackInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.fallbackInputCostPerMillionMicros ?? 0) / 1000000} /></label><label><span>{phrase("备用缓存输入价 / 百万 tokens", "Fallback cached input price / million tokens")}</span><input min={0} onChange={(event) => update(item.capability, "fallbackCachedInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.fallbackCachedInputCostPerMillionMicros ?? 0) / 1000000} /></label><label><span>{phrase("备用输出价 / 百万 tokens", "Fallback output price / million tokens")}</span><input min={0} onChange={(event) => update(item.capability, "fallbackOutputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.fallbackOutputCostPerMillionMicros ?? 0) / 1000000} /></label></div>
