...
export default function AiAdminPage() {
  ...
  return <section className="page-shell admin-shell ai-admin-page">
    ...
    <form className="ai-config-panel" onSubmit={(event) => void submit(event)}>
      ...
      <div className="ai-config-fields ai-fallback-pricing-fields"><label><span>{phrase("备用输入价 / 百万 tokens", "Fallback input price / million tokens")}</span><input min={0} onChange={(event) => update("fallbackInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.fallbackInputCostPerMillionMicros ?? 0) / 1000000} /></label><label><span>{phrase("备用输出价 / 百万 tokens", "Fallback output price / million tokens")}</span><input min={0} onChange={(event) => update("fallbackOutputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.fallbackOutputCostPerMillionMicros ?? 0) / 1000000} /></label></div>
