...
import { AiNumberInput } from "@/components/ai-number-input";
  ...
  return <section className="page-shell admin-shell ai-admin-page">
    ...
    <form className="ai-config-panel" onSubmit={(event) => void submit(event)}>
      ...
      <div className="ai-pricing-grid">
        <label><span>{phrase("输入价 / 百万 tokens", "Input price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update("inputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={draft.inputCostPerMillionMicros / 1000000} /></label>
        <label><span>{phrase("备用输入价 / 百万 tokens", "Fallback input price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update("fallbackInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={(draft.fallbackInputCostPerMillionMicros ?? 0) / 1000000} /></label>
        <label><span>{phrase("缓存输入价 / 百万 tokens", "Cached input price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update("cachedInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={(draft.cachedInputCostPerMillionMicros ?? 0) / 1000000} /></label>
        <label><span>{phrase("备用缓存输入价 / 百万 tokens", "Fallback cached input price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update("fallbackCachedInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={(draft.fallbackCachedInputCostPerMillionMicros ?? 0) / 1000000} /></label>
        <label><span className="ai-pricing-label"><span>{phrase("输出价 / 百万 tokens", "Output price / million tokens")}</span>{pricingPreset ? <button aria-label={phrase("套用模型价格预设", "Apply model pricing preset")} className="ai-pricing-preset-button" onClick={applyPricingPreset} title={phrase(`${pricingPreset.label}：套用价格参考`, `Apply ${pricingPreset.label} pricing reference`)} type="button"><Sparkles size={13} /></button> : null}</span><AiNumberInput min={0} onChange={(event) => update("outputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={draft.outputCostPerMillionMicros / 1000000} /></label>
        <label><span>{phrase("备用输出价 / 百万 tokens", "Fallback output price / million tokens")}</span><AiNumberInput min={0} onChange={(event) => update("fallbackOutputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} value={(draft.fallbackOutputCostPerMillionMicros ?? 0) / 1000000} /></label>
      ...
      <div className="ai-config-fields ai-limit-fields"><label><span>{phrase("全站并发", "Global concurrency")}</span><AiNumberInput max={recommendation.maxGlobalConcurrency} min={1} onChange={(event) => update("globalConcurrency", Number(event.target.value))} value={draft.globalConcurrency} /></label><label><span>{phrase("单用户并发", "Per-user concurrency")}</span><AiNumberInput max={recommendation.maxUserConcurrency} min={1} onChange={(event) => update("userConcurrency", Number(event.target.value))} value={draft.userConcurrency} /></label><label><span>{phrase("最大输出 tokens", "Max output tokens")}</span><AiNumberInput max={8000} min={256} onChange={(event) => update("maxOutputTokens", Number(event.target.value))} value={draft.maxOutputTokens} /></label><label><span>{phrase("请求超时（秒）", "Request timeout (seconds)")}</span><AiNumberInput max={300} min={10} onChange={(event) => update("requestTimeoutSeconds", Number(event.target.value))} value={draft.requestTimeoutSeconds} /></label><label><span>{phrase("全站每日请求上限（0 为不限）", "Global daily request limit (0 = unlimited)")}</span><AiNumberInput max={100000} min={0} onChange={(event) => update("dailyRequestLimit", Number(event.target.value))} value={draft.dailyRequestLimit} /></label><label className="ai-currency-field"><span>{phrase("计价币种", "Billing currency")}</span><GlassSelect ariaLabel={phrase("计价币种", "Billing currency")} leadingIcon={<Activity size={14} />} menuClassName="ai-currency-menu" menuPortal onChange={(value) => update("billingCurrency", value)} options={[{ value: "USD", label: "USD" }, { value: "CNY", label: "CNY" }]} value={draft.billingCurrency} /></label></div>
      <div className="ai-context-settings"><div className="ai-context-toggles"><label className="ai-config-toggle"><span>{phrase("启用知识库检索", "Enable knowledge retrieval")}</span><input checked={Boolean(draft.ragEnabled)} onChange={(event) => update("ragEnabled", event.target.checked)} type="checkbox" /></label><label className="ai-config-toggle"><span>{phrase("记录回答质量指标", "Record quality metrics")}</span><input checked={Boolean(draft.qualityEvaluationEnabled)} onChange={(event) => update("qualityEvaluationEnabled", event.target.checked)} type="checkbox" /></label></div><div className="ai-context-number-fields"><label><span>{phrase("每次最多引用片段", "Max retrieved chunks")}</span><AiNumberInput max={20} min={1} onChange={(event) => update("ragTopK", Number(event.target.value))} value={draft.ragTopK ?? 6} /></label><label><span>{phrase("保留最近消息数", "Recent messages")}</span><AiNumberInput max={80} min={4} onChange={(event) => update("contextMaxMessages", Number(event.target.value))} value={draft.contextMaxMessages ?? 24} /></label><label><span>{phrase("自动压缩阈值", "Summary threshold")}</span><AiNumberInput max={200} min={8} onChange={(event) => update("contextSummaryThreshold", Number(event.target.value))} value={draft.contextSummaryThreshold ?? 32} /></label><label><span>{phrase("上下文保留天数", "Context retention days")}</span><AiNumberInput max={3650} min={1} onChange={(event) => update("contextRetentionDays", Number(event.target.value))} value={draft.contextRetentionDays ?? 365} /></label></div></div>
