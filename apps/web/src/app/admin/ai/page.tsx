...
export default function AiAdminPage() {
  ...
  return <section className="page-shell admin-shell ai-admin-page">
    ...
    <form className="ai-config-panel" onSubmit={(event) => void submit(event)}>
      ...
      <div className="ai-config-fields ai-fallback-pricing-fields"><label><span>{phrase("备用输入价 / 百万 tokens", "Fallback input price / million tokens")}</span><input min={0} onChange={(event) => update("fallbackInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.fallbackInputCostPerMillionMicros ?? 0) / 1000000} /></label><label><span>{phrase("备用缓存输入价 / 百万 tokens", "Fallback cached input price / million tokens")}</span><input min={0} onChange={(event) => update("fallbackCachedInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.fallbackCachedInputCostPerMillionMicros ?? 0) / 1000000} /></label><label><span>{phrase("备用输出价 / 百万 tokens", "Fallback output price / million tokens")}</span><input min={0} onChange={(event) => update("fallbackOutputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.fallbackOutputCostPerMillionMicros ?? 0) / 1000000} /></label></div>
      <div className="ai-config-fields ai-limit-fields"><label><span>{phrase("全站并发", "Global concurrency")}</span><input max={recommendation.maxGlobalConcurrency} min={1} onChange={(event) => update("globalConcurrency", Number(event.target.value))} type="number" value={draft.globalConcurrency} /></label><label><span>{phrase("单用户并发", "Per-user concurrency")}</span><input max={recommendation.maxUserConcurrency} min={1} onChange={(event) => update("userConcurrency", Number(event.target.value))} type="number" value={draft.userConcurrency} /></label><label><span>{phrase("最大输出 tokens", "Max output tokens")}</span><input max={8000} min={256} onChange={(event) => update("maxOutputTokens", Number(event.target.value))} type="number" value={draft.maxOutputTokens} /></label><label><span>{phrase("请求超时（秒）", "Request timeout (seconds)")}</span><input max={300} min={10} onChange={(event) => update("requestTimeoutSeconds", Number(event.target.value))} type="number" value={draft.requestTimeoutSeconds} /></label><label><span>{phrase("全站每日请求上限（0 为不限）", "Global daily request limit (0 = unlimited)")}</span><input max={100000} min={0} onChange={(event) => update("dailyRequestLimit", Number(event.target.value))} type="number" value={draft.dailyRequestLimit} /></label><label className="ai-currency-field"><span>{phrase("计价币种", "Billing currency")}</span><GlassSelect ariaLabel={phrase("计价币种", "Billing currency")} leadingIcon={<Activity size={14} />} menuClassName="ai-currency-menu" menuPortal onChange={(value) => update("billingCurrency", value)} options={[{ value: "USD", label: "USD" }, { value: "CNY", label: "CNY" }]} value={draft.billingCurrency} /></label><label><span>{phrase("输入价 / 百万 tokens", "Input price / million tokens")}</span><input min={0} onChange={(event) => update("inputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={draft.inputCostPerMillionMicros / 1000000} /></label><label><span>{phrase("缓存输入价 / 百万 tokens", "Cached input price / million tokens")}</span><input min={0} onChange={(event) => update("cachedInputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={(draft.cachedInputCostPerMillionMicros ?? 0) / 1000000} /></label><label><span className="ai-pricing-label"><span>{phrase("输出价 / 百万 tokens", "Output price / million tokens")}</span>{pricingPreset ? <button aria-label={phrase("套用模型价格预设", "Apply model pricing preset")} className="ai-pricing-preset-button" onClick={applyPricingPreset} title={phrase(`${pricingPreset.label}：套用价格参考`, `Apply ${pricingPreset.label} pricing reference`)} type="button"><Sparkles size={13} /></button> : null}</span><input min={0} onChange={(event) => update("outputCostPerMillionMicros", Math.round(Number(event.target.value) * 1000000))} step={0.000001} type="number" value={draft.outputCostPerMillionMicros / 1000000} /></label></div>{pricingPreset ? <p className="ai-pricing-preset-note">{phrase(`${pricingPreset.note} 点击输出价格右侧图标可套用。`, `${pricingPreset.note} Use the icon beside output price to apply it.`)}</p> : null}
