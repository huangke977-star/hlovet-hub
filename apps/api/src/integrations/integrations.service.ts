...
export class IntegrationsService implements OnModuleInit, OnModuleDestroy {
  ...
  private async deliverOne(id: number) {
    ...
    try { const secret = this.crypto.decrypt(delivery.endpoint.secretEncrypted); const signature = `sha256=${createHmac("sha256", secret).update(body).digest("hex")}`; const response = await fetch(delivery.endpoint.url, { method: "POST", headers: { "content-type": "application/json", "user-agent": "HLOVET-Webhook/1.0", "x-hlovet-event": delivery.eventType, "x-hlovet-event-id": delivery.eventId, "x-lingxi-event": delivery.eventType, "x-lingxi-event-id": delivery.eventId, "idempotency-key": delivery.idempotencyKey, "x-hlovet-signature-256": signature, "x-lingxi-signature-256": signature }, body, signal: AbortSignal.timeout(8_000) }); if (!response.ok) throw new Error(`HTTP ${response.status}`); await this.prisma.externalWebhookDelivery.update({ where: { id }, data: { status: WebhookDeliveryStatus.delivered, attempts: attempt, deliveredAt: new Date(), lastError: null } }); await this.prisma.externalWebhookEndpoint.update({ where: { id: delivery.endpointId }, data: { lastDeliveredAt: new Date(), lastError: null } }); }
  ...
  private async sendExternal(row: Pick<ExternalNotificationChannel, "endpoint" | "secretEncrypted">, payload: Record<string, unknown>) { const body = JSON.stringify(payload); const headers: Record<string, string> = { "content-type": "application/json", "user-agent": "HLOVET-Notifications/1.0" }; if (row.secretEncrypted) { const signature = `sha256=${createHmac("sha256", this.crypto.decrypt(row.secretEncrypted)).update(body).digest("hex")}`; headers["x-hlovet-signature-256"] = signature; headers["x-lingxi-signature-256"] = signature; } const response = await fetch(row.endpoint, { method: "POST", headers, body, signal: AbortSignal.timeout(8_000) }); if (!response.ok) throw new Error(`HTTP ${response.status}`); }
